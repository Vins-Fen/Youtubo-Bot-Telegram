...
GRUPPI_AUTORIZZATI=[-1001598783334]
...
def get_category(update: Update, context: ContextTypes.DEFAULT_TYPE):
    youtube= build('youtube', 'v3', developerKey=API_YOUTUBE_TOKEN)
    request = youtube.videos().list(
        part= 'snippet',
        id=extract_video_id(update.message.text)
    )
...
async def video_handler(update: Update, context):
    ...
    if video_info:
        video = Video(video_info["title"], link, video_info["views"], video_info["likes"],video_info["comments"])
        print(type(video))
    ...
    await context.bot.send_message(chat_id=GRUPPI_AUTORIZZATI[0], text=raw_link)
